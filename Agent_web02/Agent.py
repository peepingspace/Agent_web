...
# ====================== 提示词 ======================
INTENT_PROMPT = """
你是意图识别机器人，**必须严格遵守以下规则，优先级从上到下，违反规则直接输出错误**：
1. 【最高优先级】只要用户问题中包含以下任意关键词：
   叶挺、新四军、南昌起义、广州起义、叶挺将军、叶挺军长
   必须输出：rag
   绝对禁止输出：code/doctor/bilibili/normal_chat
2. 包含BV/bv → 输出：bilibili
3. 代码相关 → 输出：code
4. 运动/健康/康复 → 输出：doctor
5. 其他所有情况 → 输出：normal_chat
...
def node_intent(state: AllState):
    q = state["question"]
    if any(keyword in q for keyword in ["叶挺", "新四军", "南昌起义", "广州起义", "铁军"]):
        print("✅ 检测到叶挺相关关键词，强制设置intent为rag")
        return {"intent": "rag"}
